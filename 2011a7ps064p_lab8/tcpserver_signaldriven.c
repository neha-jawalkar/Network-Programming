...
if (si->si_code==POLL_IN)
{
  ...
  fcntl (connfd[i], F_SETOWN, getpid ());
  int flags = fcntl (connfd[i], F_GETFL);	/* Get current flags */
  fcntl (connfd[i], F_SETFL, flags | O_ASYNC | O_NONBLOCK);
  fcntl (connfd[i], F_SETSIG, SIGRTMIN + 2);
}
if (sig == SIGIO)
  ...
  else if (n > 0)
{
  ...
  for(i=0;i<MAXCONN;i++)
  {
    if(connfd[i] > 0 && connfd[i] != si->si_fd)
      write(connfd[i],buf,n);
    else if(connfd[i] == si->si_fd)
      cli_state[i] = 1;
  }
  write (si->si_fd, "OK\n", 3);
}
...
for (;;)
{
  ...
  for(i=0;i<MAXCONN;i++)
  {
    ...
    {
      cli_state[i] = 0;
      write(connfd[i],HELLOMSG,HELLOMSGSZ);
    }
  }
  timeout = 0;
  alarm(TIMEOUT);
